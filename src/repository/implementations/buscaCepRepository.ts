...
const correios = async (cep) => {
    ...
    return new CepDados({ rua: logradouroDNEC, cidade: localidade, estado: uf, provider: 'correio' })
}
...
const viaCep = async (cep) => {
    ...
    return new CepDados({ rua: logradouro, cidade: localidade, estado: uf, provider: 'viacep' })
}
